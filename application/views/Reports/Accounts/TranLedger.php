    ...
    <section class="content">
      ...
      <div class="box box-info">
        <div class="box-header with-border">
          <h3 class="box-title">Transaction Ledger</h3>
          <div class="box-tools pull-right">
          </div>
        </div>
        <div class="box-body">
        <form action="<?= base_url('/tran_ledger_report'); ?>" method="post" id="ReportForm">
            ...
            <div class="row">
              ...
              <?php endif; ?>
                <div class="col-md-2">
                    <div class="form-group">
                        <label for="branch">Accounts Head </label>
                        <select class="form-control" name="tran_head_id" id="tran_head_id" required>
                          <option value="">Select Head</option>
                          <?php
                            $sub_head_id = 0;
                            foreach($headList as $v) {
                              if($sub_head_id != $v->sub_head_id){
                                if(!empty($output)) echo '</optgroup>';
                                echo '<optgroup label="'.$v->sub_head_name.'">';
                                $sub_head_id = $v->sub_head_id;
                              }
...
<script type="text/javascript">
  ...
  $("#print-button").on('click', function(){
    ...
    if($("#tran_head_id").val() == '') {
      alert("Please select accounts head");
      return false;
    }
    if($("#from_date").val() == '') {
